...
| 目錄 | 內容 | 狀態 |
| --- | --- | --- |
| `fill-shulin-forms/` | 輸出端：回填樹林區普通住宅用地的三份查估書表 | 主線完成 |
| `import-survey-xlsx/` | 輸入端：匯入填好的勘查表 xlsx | 主線完成 |

兩份是同一條鏈的兩端。`import-survey-xlsx` 另外定下三種輸入格式收斂到同一層的
架構（PDF 靠座標、xlsx 靠格位、圖片走 OCR，三者輸出同一種勘查事實格式），
所以 `kernel` 不必知道資料從哪來，而且其中一條壞掉不影響另外兩條。
