...
- 新北市手冊 93 頁全文 OCR：用 `python scripts/ocr-manual.py` 產生，
  輸出到官方文件目錄旁邊（不進版控，等同官方文件內容）。
  那份手冊內文抽不出文字，OCR 用來定位與搜尋，引用條文要回去看原圖。
...
簡報腳本與預備問答不進版控（不是原始碼，且內容隨場合調整），
存在專案外的私人筆記目錄。repo 裡留的是兩支現場跑的腳本：
