...
#: 表3 每個地價區段該動的 98 格。四張表結構相同，值不同。
#:
#: 23 格是量測值與勾選（`E31`／`E32` 土地改良與 `Q44` 土地利用現況是覆寫
#: 勾選符號），56 格是 28 個細項的優劣等級與總級數，
#: 19 格是本案沒有該設施的「名稱：」欄填「無」。
#:
...
TABLE3_EXPECTED = {
    ...
    # 本案沒有該設施的細項，「名稱：」欄填「無」（19 格）
    cell
    for cells in layout.TABLE3_ABSENT_NAME_CELLS.values()
    for cell in cells
}
...
#: 表5-1 動過的總格數 = 116 級數 + 116 等級文字 + 87 修正率 + 24 小計
#: + 3 總修正數 + 8 表頭 + 1 全案備註
#:
#: 等級文字那 116 格是後來補的。優劣等級在書表上是兩欄（左欄級數、
#: 右欄等級文字），依新北查估書表製作手冊第 5 章第 42 頁，
#: 而表頭 C4:D4 合併讓它看起來像一欄。
TABLE5_1_TOTAL = 355
