...
#: 等級文字欄填量測值而不是優劣文字的細項。
#:
#: 新北市查估書表製作手冊的兩個範例都這樣填：第 52 頁商業用地範例的
#: 都市計畫是「1 內」、使用分區是「1 商業區」；第 56 頁其他用地範例的
#: 都市計畫是「1 內」、使用分區是「4 溝渠用地」。
#: 其餘細項（道路寬度、日照、地勢那些程度型的）兩個範例都填優劣文字。
#:
#: 理由推測是這兩項有明確的類別名稱，填類別比填「優」有資訊量，
#: 而且手冊要求表5-1 與勘查表相符，填量測值就直接相符。
#:
#: 內政部的查估書表範本這兩項填「優」，與新北市手冊不一致。
#: 本案是新北市樹林區的案件，所以從新北市手冊。
TABLE5_1_LABEL_FROM_VALUE = (
