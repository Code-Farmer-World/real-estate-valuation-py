...
### `POST /api/survey/xlsx`（產出模式，2026-09-12 新增）

上傳填好的表3 勘查表 xlsx，算出表5-1 與表4，並產出可交件的書表。

與 `/api/review` 方向相反。審查那條是「已經有填好的表，重算去比對」，這條是
「表是空的，算出每一格該填什麼」。正式題目的表5-1 與表4 空白待填，而勘查表沒有
優劣等級這一欄，所以審查那條在那個案子沒有對照對象。

`data` 內的欄位：

| 欄位 | 內容 |
| --- | --- |
| `id` | 這次產出的 token，下載連結會用到 |
| `case_id` / `ruleset_id` / `benchmark` / `comparables` | 案件與規則集資訊 |
| `cell_counts` | 填出來的格數（116 等級／87 修正率／24 小計／3 總修正數） |
| `table5_1` | `groups`、`factor_ids`、`grades`、`subtotals`、`totals` |
| `table4` | 區域因素調整百分率、絕對值加總、相近程度、權重、試算價格、比準地比較價格、比準地地價 |
| `premise` | 個別因素以 0 計的前提說明。**少了它試算價格會被當成完整答案** |
| `evidence` | 依據鏈 87 列。每列附量測值、級距條文、基準表頁碼、矩陣查表結果與一句敘述 |
| `verification` | 自我驗證的十個項目 |
| `read_warnings` | 該有值卻讀到空白的欄位。不中斷但要讓使用者看到 |
| `files` | 六個檔案的下載連結（表3、表5-1 與表4 各兩版、驗證報告） |

錯誤碼：非 xlsx 回 422；壞掉的 xlsx 或不是本案的勘查表回 400（訊息說明原因）；
伺服器找不到官方空白範本目錄回 503（要設 `VALUATION_TEMPLATE_DIR`）。

`evidence` 的敘述由規則引擎產生，**不經過任何模型**。徵收案會被訴願，
「為什麼這一格是第 5 級」必須答得出來而且答案要每次都一樣。

### `GET /api/survey/{token}/{filename}`

下載產出的 xlsx 或驗證報告。與 `/api/forms/{token}/{filename}` 一樣不套信封，
回傳檔案本身。檔名走白名單，拒絕路徑穿越。
