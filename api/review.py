...
刻意誠實的一點：能查到什麼取決於規則集補到哪裡。區域因素目前只有 5/28 項，
所以第一層只查得動 5 項，其餘 23 項會列在 `not_checkable` 裡並說明原因——
不會假裝「全部通過」。
"""
...
def review(
    ...
    if t1 and rs_regional is not None:
        layers["table1_internal"], skipped = _check_table1_internal(t1, rs_regional)
    ...
    if t4:
        findings, price_impact = _check_table4(t4, t52, rs_individual)
...
def _check_table1_internal(
    ...
    for factor_id, cell in t1["grades"].items():
        ...
        value = survey.get("numeric")
        if value is None:
            value = survey.get("text")
        if value is None:
            skipped.append(
                {
                    "layer": "table1_internal",
                    "factor_id": factor_id,
                    "reason": "表1 這一格沒有可判讀的量測值（原文：%s）"
                    % (survey.get("raw") or ""),
                }
            )
            continue

        grade = classify(rs[factor_id], value)
            ...
            findings.append(
                {
                    ...
                    "basis": "表1 量測值 %s；%s" % (value, grade.reason),
                }
            )
